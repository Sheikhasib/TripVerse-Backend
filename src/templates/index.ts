...
// Renders an EJS email template by name. The template directory is resolved at
// runtime with fallbacks so it works in every host:
//   - dev (`tsx watch`) and local `dist` run with cwd = project root → src/templates
